...
export function forecast(w:Workspace,p:Point,now=new Date()){const history=historyFor(w,p);if(p.frequency==='Única'||!history.length)return null;const period={Semanal:7,Quinzenal:14,Mensal:30}[p.frequency]??7;const last=new Date(history[history.length-1].date);const next=new Date(last);next.setDate(last.getDate()+period);while(next<now)next.setDate(next.getDate()+period);return {average:history.reduce((s,h)=>s+h.kg,0)/history.length,next,period,samples:history.length};}
