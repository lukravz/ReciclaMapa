...
export default function LocationEditor({address,onAddress,position,onPosition,confirmed,onConfirmed,residential=false,requireAddress=true}:{requireAddress?:boolean;address:AddressDraft;onAddress:(value:AddressDraft)=>void;position:Coordinates|null;onPosition:(value:Coordinates|null)=>void;confirmed:boolean;onConfirmed:(value:boolean)=>void;residential?:boolean}) {
  ...
  async function searchCep(){cancel();const cep=normalizeCep(address.postcode);if(!cep){setError('Informe um CEP com 8 dígitos.');return;}setBusy(true);setError('');const current=sequence.current;try{const value=await api<{cep:string;logradouro:string;bairro:string;cidade:string;estado:string}>('cep/'+cep);if(current!==sequence.current)return;onAddress({...address,postcode:value.cep,street:value.logradouro,region:value.bairro,city:value.cidade,state:value.estado});onConfirmed(false);}catch(e){if(current===sequence.current)setError((e as Error).message);}finally{if(current===sequence.current)setBusy(false);}}
  ...
  return <div className="location-editor"><div className="form-grid">
    ...
    <div className="address-search"><button type="button" className="btn light" disabled={busy} onClick={searchCep}>Preencher pelo CEP</button></div><div className="address-search"><button type="button" className="btn light" onClick={search} disabled={busy||!address.city.trim()||!address.state.trim()||(!address.street.trim()&&!address.region.trim())}>{busy?<LoaderCircle className="spinning" size={17}/>:<Search size={17}/>} {busy?'Buscando endereço...':'Buscar endereço no mapa'}</button></div>
