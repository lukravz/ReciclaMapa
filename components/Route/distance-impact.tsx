...
export default function DistanceImpact({workspace}:{workspace:Workspace}) {
  ...
  const items=[{label:'Rotas concluídas',value:String(workspace.collections.length),unit:'rotas',Icon:Route,note:`${routed.length} com comparação viária`},{label:'Sem otimização',value:routed.length?num(before):'—',unit:'km',Icon:Milestone,note:'Ordem original · OSRM'},{label:'Após otimização',value:routed.length?num(after):'—',unit:'km',Icon:Navigation,note:'Distância planejada · OSRM'},{label:'Km evitados (estimativa)',value:routed.length?num(Math.max(0,before-after)):'—',unit:'km',Icon:TrendingDown,note:'Diferença entre rotas viárias calculadas'}];
